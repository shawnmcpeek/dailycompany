...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "Benedict Daily");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "Benedict Daily");
  }

  gtk_window_set_default_size(window, 1280, 720);

  // Window / taskbar icon (installed next to the binary under data/).
  {
    g_autoptr(GError) error = nullptr;
    g_autofree gchar* exe_link = g_file_read_link("/proc/self/exe", nullptr);
    g_autofree gchar* exe_dir =
        g_path_get_dirname(exe_link != nullptr ? exe_link : ".");
